...
auto events_longest_asleep(string[] events)
{
	...
	if (!type.cmp("falls asleep")) {
		...
		if (sleepy_guard == null || sleepy_minute == -1 ||
		        sleepy_guard.sleep_count[sleepy_minute] < guards[cur_guard].sleep_count[i]) {
...
unittest
{
    ...
    test_guard("tests/3.txt", tuple!("id", "time")(1238, 15));
    writeln("Success.");
    write("Sleep twice on same day... ");
    test_guard("tests/4.txt", tuple!("id", "time")(1238, 15));
    writeln("Success.");
    write("Sleep twice different day... ");
    test_guard("tests/5.txt", tuple!("id", "time")(1238, 15));
    writeln("Success.");
    write("Example input... ");
    test_guard("tests/6.txt", tuple!("id", "time")(99, 45));
    writeln("Success.");
}
